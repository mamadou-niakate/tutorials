...
export function TasksList({ tasks, title, id }) {
  ...
  const deleteList = () => {
    dispatch({ type: DELETE_TASKS_LIST, payload: id });
  };
  ...
  const editList = () => {
    ...
    dispatch({ type: OPEN_TASKS_LIST_FORM, payload: true });
  };
  ...
  return (
    <List key={title}>
      ...
      {tasks?.map((task) => {
        return <TasksListItem key={task.id} task={task} listId={id} />;
      })}
      {openTaskForm ? (
        <TasksListItemForm
